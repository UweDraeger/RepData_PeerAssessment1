...
```
The most active 5-minute-interval starts `r maxsteps$interval`.
...
There are in total `r totalnum` intervals with missing observations which are limited to 
eight days with no measurement whatsoever. 
It might be required to impute missing values.
Given observed intraday activity pattern it seems prudent to replace missing values with the mean for that 5-minute interval.
